...
func (c *FaceboxClient) CheckMulti(ctx context.Context, requests []*CheckRequest) ([]*CheckResponse, error) {
	...
	for i := range requests {
		if requests[i].fileImage != nil {
			fieldName := "_files[" + strconv.Itoa(files) + "]"
			requests[i].Image = remototypes.NewFile(fieldName)
			f, err := w.CreateFormFile(fieldName, "todo.dat")
			if err != nil {
				return nil, errors.Wrap(err, "Facebox.Check: create form file")
			}
			if _, err := io.Copy(f, requests[i].fileImage); err != nil {
				return nil, errors.Wrap(err, "Facebox.Check: reading file")
			}
			files++
			select {
			case <-ctx.Done():
